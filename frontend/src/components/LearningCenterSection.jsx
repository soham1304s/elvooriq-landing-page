...
const LearningCenterSection = () => {
  ...
  useEffect(() => {
    ...
    socket.on('video_updated', handleVideoUpdated);
    socket.on('video_deleted', handleVideoDeleted);

    return () => {
      socket.off('landing:featured_videos_update', handleVideosUpdate);
      socket.off('video_added', handleVideoAdded);
      socket.off('video_updated', handleVideoUpdated);
      socket.off('video_deleted', handleVideoDeleted);
    };
  }, []);

  const filteredVideos = selectedCategory === 'All' 
    ? videos 
    : videos.filter(v => (v.category || '').toLowerCase() === selectedCategory.toLowerCase() || (v.title || '').toLowerCase().includes(selectedCategory.toLowerCase()));
      ...
      <div className="learning-container container">
        ...
        >
          {loading ? (
            ...
            <motion.div variants={fadeInUp} className="video-card" style={{ gridColumn: '1 / -1', textAlign: 'center' }}>
              No videos found in "{selectedCategory}". Check back soon for masterclass uploads.
            </motion.div>
          ) : (
