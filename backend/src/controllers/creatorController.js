...
exports.addCreator = async (req, res) => {
  try {
    ...
    if (req.io) {
      const updatedList = await prisma.featuredCreator.findMany({ where: { isActive: true }, orderBy: { createdAt: 'desc' } });
      req.io.emit('content:updated', updatedList);
      req.io.emit('content:updated:event', { type: 'creator', action: 'add', data: newCreator });
    }
...
exports.deleteCreator = async (req, res) => {
  try {
    ...
    if (req.io) {
      const updatedList = await prisma.featuredCreator.findMany({ where: { isActive: true }, orderBy: { createdAt: 'desc' } });
      req.io.emit('content:updated', updatedList);
      req.io.emit('content:updated:event', { type: 'creator', action: 'delete', id });
    }
